...
function makeTableFilterable(table) {
    ...
    inputbox.oninput = function () {
        ...
        var reg2 = /<span style=\"background-color: yellow\">|<\/span>/g;  // 把<span>标签去掉 // 注意style后面不能有空格！
        var tmpreg = reg;
        for (var i = 0; i < trset.length; i++) {
            ...
            if (status === false)  trset[i].style.display = 'none';
            else  trset[i].style.display = 'table-row';
        }
    }
}
